...
export default function CarShowcase({ category = 'automobiles' }) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      timeline.to('.car-wrapper', { y: '0vh', ease: 'power3.out', duration: 2 }, 0)
        .to('.car-img', { scale: 1, filter: 'blur(0px)', duration: 1.5, ease: 'power2.out' }, 0.5)
        .to('.car-wrapper', { x: '15vw', ease: 'power2.inOut', duration: 1.5 }, 2.5)
        .to('.car-text-panel', { opacity: 1, x: 0, ease: 'power2.out', duration: 1.5 }, 2.5)
        .to({}, { duration: 1.5 })
        .to('.car-text-panel', { opacity: 0, x: -50, duration: 1 }, 5.5)
        .to('.car-wrapper', { y: '-120vh', ease: 'power3.in', duration: 2 }, 5.5)
        .to('.car-img', { scale: 0.85, filter: 'blur(1px)', duration: 1.5, ease: 'power2.in' }, 6);
    }, containerRef);
