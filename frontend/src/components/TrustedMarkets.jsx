...
const brandsDataStore = {
  ...
  "personal-care": {  
    ...
    brands: [
      { name: "Gillette", desc: "Globally recognized daily skincare, lotions, and deep moisturizing body washes.", img: logo1 },
      { name: " Silver Max", desc: "High-quality grooming products for a modern look and feel.", img: logo2 },
      { name: "Dorco", desc: "Premium personal care products for a modern lifestyle.", img: logo3 },
      { name: " NUBIA", desc: "Award-winning fragrance house specializing in deep, complex oriental scent profiles.", img: logo4 },
      { name: "SANTALUXE", desc: "Premium luxury collections crafted from pure, ethically sourced organic extracts.", img: logo6 },
      { name: " DUBEY PERFUMERY", desc: "Master perfumers creating world-class attars, essential oils, and concentrates.", img: logo6 },
      { name: "Bakhoor", desc: "Traditional Arabian fragrance with a modern twist.", img: logo6 }
    ]
  },
  "silvermax": {
...
export default function TrustedMarkets({ category = "automobiles" }) {
  ...
  // Added support for exactly 7 columns so all brands fit on a single line on desktop
  const numBrands = data.brands.length;
  const desktopGridClass = 
    numBrands === 7 ? 'lg:grid-cols-7' :
  ...
  return (
    <section ref={containerRef} className="w-full bg-white py-10 md:py-10 font-sans">
      {/* Changed max-w-[1400px] to max-w-[1536px] (8xl) for a wider container */}
      <div className="max-w-[1536px] mx-auto px-[clamp(1.5rem,5vw,4rem)] flex flex-col items-center">
        
        {/* ======================================================= */}
        {/* SECTION HEADER                                          */}
        {/* ======================================================= */}
        <div className="brand-header-anim flex flex-col items-center text-center mb-6">
        ...
        <div className={`brand-grid w-full grid grid-cols-2 md:grid-cols-3 ${desktopGridClass} gap-4 md:gap-6`}>
          ...
          {data.brands.map((brand, index) => (
            <div 
              key={index} 
              className="brand-card group relative bg-white border border-slate-200/80 rounded-xl h-[110px] md:h-[110px] flex items-center justify-center overflow-hidden shadow-sm hover:shadow-2xl transition-shadow duration-500 cursor-pointer"
            >
              {/* --- DEFAULT STATE: THE LOGO --- */}
              <div className="w-[45%] md:w-[50%] h-10 md:h-12 flex items-center justify-center transform transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] group-hover:-translate-y-6 md:group-hover:-translate-y-8">
                {brand.img ? (
                  <img 
                    src={brand.img} 
                    alt={brand.name} 
                  ...
                  <h3 className="text-[#1c2331] font-black text-[9px] sm:text-[10px] md:text-xs tracking-widest uppercase text-center leading-tight">
                    {brand.name}
                  </h3>
                )}
              </div>
              ...
              <div className="absolute inset-x-0 bottom-0 bg-[#111111] p-4 md:p-5 translate-y-[101%] group-hover:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col justify-center border-t border-orange-500/30">
                ...
                {/* Text slightly smaller so it never overflows when packed into 6 or 7 columns */}
