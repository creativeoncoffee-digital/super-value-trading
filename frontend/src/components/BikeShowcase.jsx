  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add("(min-width: 768px)", () => {
        ...
        gsap.set('.bike-hero-img', { scale: 1.4, y: '10vh', x: '0vw', opacity: 0 });
        gsap.set('.bike-bg-glow', { scale: 0.5, opacity: 0, x: '0vw' });
        // Use yPercent for perfect vertical centering on desktop
        gsap.set('.bike-text-content', { opacity: 0, x: -60, yPercent: -50, y: 0 });

        tl.to('.bike-hero-img', { opacity: 1, y: '0vh', duration: 1.5, ease: 'power2.out' })
          ...
          .to('.bike-text-content', { opacity: 1, x: 0, yPercent: -50, duration: 2, ease: 'power3.out' }, "-=1.5")
          .to({}, { duration: 3 })
          .to('.bike-hero-img', { opacity: 0, y: '-10vh', scale: 0.95, duration: 1.5, ease: 'power2.in' })
          .to('.bike-bg-glow', { opacity: 0, duration: 1.5 }, "<")
          .to('.bike-text-content', { opacity: 0, yPercent: -70, duration: 1.5, ease: 'power2.in' }, "<");
      });
      ...
      mm.add("(max-width: 767px)", () => {
        ...
        // Initial State: Bike bottom, Text hidden
        gsap.set('.bike-hero-img', { y: '100vh', x: '0vw', opacity: 1, scale: 1, rotation: 0 });
        gsap.set('.bike-bg-glow', { y: '100vh', x: '0vw', scale: 0.5, opacity: 0 });
        gsap.set('.bike-text-content', { opacity: 0, y: 30, x: 0, yPercent: 0 });

        mobileTl
          // 1. Bike drives in to the upper-middle of the screen (-15vh) to leave room for text
          .to('.bike-hero-img', { y: '-15vh', duration: 2, ease: 'power2.out' })
          .to('.bike-bg-glow', { opacity: 0.6, scale: 1, y: '-15vh', duration: 2, ease: 'power2.out' }, "<")
          
          // 2. Text slides up from the very bottom
          .to('.bike-text-content', { opacity: 1, y: 0, duration: 1 })
          
          // 3. Pause while the user reads
          .to({}, { duration: 2.5 })
          
          // 4. Text fades out and slightly up
          .to('.bike-text-content', { opacity: 0, y: -20, duration: 1 })
          
          // 5. Bike drives off through the top
          .to('.bike-hero-img', { y: '-100vh', duration: 2, ease: 'power2.in' })
          .to('.bike-bg-glow', { y: '-100vh', opacity: 0, duration: 2, ease: 'power2.in' }, "<");
      });
  ...
  return (
    <section ref={containerRef} className={`relative w-full h-[100vh] ${data.backgroundClass || 'bg-[#0A101D]'} font-sans overflow-hidden`}>
      ...
      <div className="absolute inset-0 w-full h-full flex items-center justify-center">
        ...
        <div className={`bike-bg-glow absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vw] md:w-[50vw] h-[80vw] md:h-[50vw] ${data.glowClass || 'bg-blue-500/20'} rounded-full blur-[100px] md:blur-[120px] pointer-events-none z-10`}></div>
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none w-full max-w-[800px] flex justify-center">
          <img
            ...
            className="bike-hero-img w-[80%] md:w-[95%] max-h-[50vh] md:max-h-[60vh] object-contain drop-shadow-[0_40px_80px_rgba(0,0,0,0.8)]"
          />
        </div>

        {/* TEXT CONTENT: Locked to bottom-[5%] on mobile, centered on desktop */}
        <div className="bike-text-content absolute bottom-[5%] pb-6 md:pb-0 md:bottom-auto md:top-1/2 left-[5%] md:left-[8%] w-[90%] md:w-[45%] flex flex-col items-start justify-center z-40 pointer-events-auto">
